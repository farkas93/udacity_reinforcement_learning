...
from models.cnn_model import QNetwork
...
class DQNAgent():
    ...
    def __init__(self, state_shape, action_size, seed):
        ...
        Params
        ======
            state_shape (shape): dimensions of the input image
        ...
        # Q-Network
        self.qnetwork_local = QNetwork(action_size=self.action_size, seed=seed).to(device)
        self.qnetwork_target = QNetwork(action_size=self.action_size, seed=seed).to(device)
        self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=LR)
        print(f"shape2: {state_shape}", flush=True)
        summary(self.qnetwork_local, (84, 84, 3))
